...
std::istream &operator>>(std::istream &s, Variable &v)
{
	...
	s >> v.name >> v.layer >> num_vals >> std::ws;
...
{
	return s >> MultiBlock<VarVal>(e.precs) >> e.var >> e.pre >> e.post;
}
...
std::istream &operator>>(std::istream &s, Operator &op)
{
	...
	s >> MultiBlock<VarVal>(op.prevails) >> MultiBlock<Effect>(op.effects);
	s >> op.cost;
	s >> end;
...
struct FDParser
{
	...
	void parse()
	{
		s >> Block<int>(version) >> Block<int>(metric) >> MultiBlock<Variable>(vars) >> MultiBlock<MutexGroup>(mutexes);

		NMultiBlock<unsigned> init_parser(vars.size(), init);
		MultiBlock<VarVal> goal_parser(goal);
		s >> Block<decltype(init_parser)>(init_parser) >> Block<decltype(goal_parser)>(goal_parser) >> MultiBlock<Operator>(operators) >> MultiBlock<Axiom>(axioms);
	}
