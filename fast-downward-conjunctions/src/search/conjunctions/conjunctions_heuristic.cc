...
#pragma warning(push)
#pragma warning( \
    default : 4800 4512 4706 4100 4127 4702 4239 4996 4456 4458 4505)
#endif
...
ConjunctionsHeuristic::ConjunctionsHeuristic(const options::Options& opts)
    : Heuristic(opts),
      ...
      counter_groups_by_regression(
          *task,
          [](const std::vector<FactPair>&) { return COUNTER_GROUP_NONE; }),
      dummy_precondition(FactSet(), false),
      empty_counter_group_index(COUNTER_GROUP_NONE),
      initial_state(true),
      strategy(opts.contains("strategy")
                   ? opts.get<std::shared_ptr<ConjunctionGenerationStrategy>>(
                         "strategy")
                   : nullptr),
      novelty_heuristics(),
      ...
      best_supporter_function(
          BestSupporterFunction(opts.get_enum("best_supporter"))),
      tie_breaking(TieBreaking(opts.get_enum("tie_breaking"))),
      rng(opts.get<int>("seed") == -1
              ? utils::RandomNumberGenerator()
              : utils::RandomNumberGenerator(opts.get<int>("seed"))),
      num_singletons(0),
      num_counters(0),
  ...
  std::cout << "best supporter function: " << best_supporter_function
            << std::endl;
  std::cout << "no relaxed plan extraction: " << no_relaxed_plan_extraction
            << std::endl;
  ...
  std::cout << "The original task would have " << num_singleton_counters
            << " counters." << std::endl;
  std::cout << "Finished hcff initialization, t = " << initialization_timer
            << std::endl;
}
...
ConjunctionsHeuristic::~ConjunctionsHeuristic() {
  ...
  for (auto conjunction : conjunctions) delete conjunction;
  for (auto action : actions) delete action;
  delete end_action;
}

// heuristic interface
auto ConjunctionsHeuristic::compute_result(EvaluationContext& eval_context)
    -> EvaluationResult {
  if (strategy) {
    auto conjunction_generation_event =
        initial_state ? ConjunctionGenerationStrategy::Event::INITIALIZATION
                      : ConjunctionGenerationStrategy::Event::STEP;
    initial_state = false;
    auto conjunction_generation_result = strategy->modify_conjunctions(
        *this, conjunction_generation_event, *task, eval_context, nullptr);
    if (conjunction_generation_result ==
        ConjunctionGenerationStrategy::Result::DEAD_END) {
      if (cache_h_values) {
    ...
    if (cache_h_values &&
        (strategy->modifies_conjunctions_at_event(
             ConjunctionGenerationStrategy::Event::STEP) ||
         strategy->modifies_conjunctions_at_event(
...
auto ConjunctionsHeuristic::compute_heuristic(const State& state) -> int {
  if (!is_last_bsg_valid_for_state(state)) {
    ...
    best_supporter_time +=
        std::chrono::steady_clock::now() - best_supporter_start_time;
    ...
    relaxed_plan_extraction_time +=
        std::chrono::steady_clock::now() - relaxed_plan_extraction_start_time;
  ...
  // NOTE: returning real cost even if PLUSONE is used, because it's usually
  // only used to
...
auto ConjunctionsHeuristic::is_last_bsg_valid_for_state(
    const State& state) const -> bool {
...
auto ConjunctionsHeuristic::get_pairwise_conditional_effect_mutexes(
  ...
  auto set_mutex = [&mutexes](auto i, auto j) {
    mutexes[i][j] = true;
    mutexes[j][i] = true;
  };
  for (auto i = 0u; i < conditional_effects.size(); ++i) {
    for (auto j = i + 1; j < conditional_effects.size(); ++j) {
      ...
      if (task->are_facts_mutex(conditional_effects[i].first,
                                conditional_effects[j].first)) {
        set_mutex(i, j);
        continue;
      }
      ...
      if (std::any_of(std::begin(conditional_effects[i].second),
                      ...
                      [&conditional_effects, j, this](const auto& pre1) {
                        return std::any_of(
                            std::begin(conditional_effects[j].second),
        ...
        set_mutex(i, j);
      // NOTE: we intentionally do NOT check if any prevails are mutex with
      // effects here, because
      // adding other conditional effects that overwrite the prevail can resolve
      // the mutex
    }
  }
  return mutexes;
...
void ConjunctionsHeuristic::get_conditional_effect_subsets_recursive(
  ...
  assert(std::find(std::begin(subsets), std::end(subsets),
                   std::vector<std::size_t>()) != std::end(subsets));
  if (current_index >= conditional_effects.size()) return;
  const auto size = subsets.size();
  for (auto i = 0u; i < size; ++i) {
    const auto& subset = subsets[i];
    if (std::any_of(std::begin(subset), std::end(subset),
                    [&conditional_effects, &conditional_effect_mutexes,
                     current_index](auto other_index) {
                      return conditional_effect_mutexes[current_index]
                                                       [other_index] ||
                             conditional_effects[current_index].first ==
                                 conditional_effects[other_index].first;
                    }))
  ...
  get_conditional_effect_subsets_recursive(conditional_effects,
                                           current_index + 1,
                                           conditional_effect_mutexes, subsets);
}

auto ConjunctionsHeuristic::get_action_instances(
    const FactSet& preconditions, const FactSet& effects,
    const std::vector<std::pair<FactPair, FactSet>>& conditional_effects) const
  ...
  const auto mutexes =
      get_pairwise_conditional_effect_mutexes(conditional_effects);
  auto conditional_effect_subset_indices =
      std::vector<std::vector<std::size_t>>();
  // add the empty set
  conditional_effect_subset_indices.emplace_back();
  get_conditional_effect_subsets_recursive(conditional_effects, 0, mutexes,
                                           conditional_effect_subset_indices);
  ...
  for (const auto& subset_indices : conditional_effect_subset_indices) {
    ...
    auto is_prevail_and_mutex_with_any_effect = [&effects, &conditional_effects,
                                                 &subset_indices, this](
        const auto& precondition) {
      if (std::any_of(std::begin(subset_indices), std::end(subset_indices),
                      [&conditional_effects, &precondition](auto other_index) {
                        return conditional_effects[other_index].first.var ==
                               precondition.var;
                      }) ||
          std::any_of(std::begin(effects), std::end(effects),
                      [&precondition](const auto& effect) {
      ...
      return std::any_of(
                 std::begin(subset_indices), std::end(subset_indices),
                 [&conditional_effects, &precondition, this](auto other_index) {
                   return task->are_facts_mutex(
                       precondition, conditional_effects[other_index].first);
                 }) ||
             std::any_of(std::begin(effects), std::end(effects),
                         [&precondition, this](const auto& effect) {
                           return task->are_facts_mutex(precondition, effect);
                         });
    };
    if (std::any_of(std::begin(subset_indices), std::end(subset_indices),
                    [&conditional_effects,
                     is_prevail_and_mutex_with_any_effect](auto index) {
                      return std::any_of(
                          std::begin(conditional_effects[index].second),
                          std::end(conditional_effects[index].second),
                          is_prevail_and_mutex_with_any_effect);
                    }) ||
        std::any_of(std::begin(preconditions), std::end(preconditions),
                    is_prevail_and_mutex_with_any_effect))
      continue;
    auto combined_preconditions = preconditions;
    auto combined_effects = effects;
    for (auto index : subset_indices) {
      combined_preconditions.insert(
          std::end(combined_preconditions),
          std::begin(conditional_effects[index].second),
          std::end(conditional_effects[index].second));
      combined_effects.push_back(conditional_effects[index].first);
    }
    std::sort(std::begin(combined_preconditions) + preconditions.size(),
              std::end(combined_preconditions));
    combined_preconditions.erase(
        std::unique(std::begin(combined_preconditions) + preconditions.size(),
                    std::end(combined_preconditions)),
        std::end(combined_preconditions));
    std::inplace_merge(
        std::begin(combined_preconditions),
        std::begin(combined_preconditions) + preconditions.size(),
        std::end(combined_preconditions));
    std::sort(std::begin(combined_effects) + effects.size(),
              std::end(combined_effects));
    std::inplace_merge(std::begin(combined_effects),
                       std::begin(combined_effects) + effects.size(),
                       std::end(combined_effects));
    assert(
        std::unique(std::begin(combined_effects), std::end(combined_effects)) ==
        std::end(combined_effects));
    action_instances.emplace_back(std::move(combined_preconditions),
                                  std::move(combined_effects));
  }
  ...
  for (auto op_proxy : task_proxy.get_operators()) {
    ...
    for (auto i = 0u; i < op_proxy.get_effects().size(); ++i) {
      ...
      if (effect.get_conditions().empty()) {
        ...
        conditional_effects.emplace_back(effect.get_fact().get_pair(),
                                         conditions);
      }
    }
...
#pragma warning(pop)
    ...
    auto action_instances =
        get_action_instances(preconditions, effects, conditional_effects);
    assert(!action_instances.empty());
    for (auto&& action_instance : action_instances) {
      assert(std::is_sorted(std::begin(action_instance.first),
                            std::end(action_instance.first)));
      assert(std::is_sorted(std::begin(action_instance.second),
                            std::end(action_instance.second)));
      actions.push_back(
          new Action(id++, op_proxy, std::move(action_instance.first),
                     std::move(action_instance.second), op_proxy.get_cost()));
  ...
  for (auto& actions_by_effect_variable : actions_by_effects)
    for (auto& actions_by_effect_value : actions_by_effect_variable)
      std::sort(std::begin(actions_by_effect_value),
                std::end(actions_by_effect_value));

#ifndef NDEBUG
  std::cout << "Instantiated " << actions.size() << " actions." << std::endl;
  std::cout << "Original number of actions: " << task->get_num_operators()
            << std::endl;
  std::cout << "Complete multiplication of conditional effects would yield "
            << naive_num_actions << std::endl;
#endif
...
void ConjunctionsHeuristic::initialize_conjunctions_containing_fact() {
  ...
  for (auto variable_domain : g_variable_domain)
    conjunctions_containing_fact.emplace_back(
        std::vector<std::vector<Conjunction*>>(variable_domain));
}

// best supporter functions
auto ConjunctionsHeuristic::compute_best_supporter_function(const State& state)
    -> cost_t {
  reset_heuristic();
...
void ConjunctionsHeuristic::reset_heuristic() {
  for (auto conjunction : conjunctions) conjunction->reset();
  for (auto& counter_group : counter_groups) counter_group.reset();
...
void ConjunctionsHeuristic::print_overflow_warning() {
  ...
  if (!printed_warning) {
    ...
    std::cout << "WARNING: integer overflow during the computation of h^Cadd"
              << std::endl;
  }
}
...
auto ConjunctionsHeuristic::compute_hcadd(const State& state) -> cost_t {
  ...
  auto queue = std::priority_queue<QueueEntry, std::vector<QueueEntry>,
                                   decltype(compare_cost)>(compare_cost);

  auto enqueue_if_necessary = [&queue](Conjunction* conjunction, cost_t cost,
                                       const Action* supporter) {
  ...
  while (!queue.empty()) {
    ...
    if (conjunction->cost < cost) continue;

    if (conjunction->is_subgoal) {
      h += conjunction->cost;
      if (--num_open_goals == 0) return h;
    }
    ...
    for (auto counter_group_index : triggered) {
      ...
      if (--counter_group.value == 0) {
        assert(
            counter_group.cost ==
            std::accumulate(std::begin(counter_group.regression_conjunctions),
                            std::end(counter_group.regression_conjunctions),
                            static_cast<cost_t>(0),
                            [](auto sum, const auto regression_conjunction) {
                              return sum + regression_conjunction->cost;
                            }));
        for (const auto& member : counter_group.group)
          enqueue_if_necessary(member.second,
                               counter_group.cost + member.first->cost,
                               member.first);
      }
...
auto ConjunctionsHeuristic::compute_hcadd_alternative(const State& state)
    -> cost_t {
  ...
  auto enqueue_if_necessary = [&priority_map, &queue](
      Conjunction* conjunction, cost_t cost, const Action* supporter, int i) {
    if (conjunction->cost == -1 || conjunction->cost > cost) {
      conjunction->cost = cost;
      conjunction->supporters.clear();
      ...
      if (it == priority_map.end()) {
        ...
        auto sorting = [](const QueueElement& left, const QueueElement& right) {
          return left.first < right.first;
        };
        std::inplace_merge(queue.begin() + i, queue.end() - 1, queue.end(),
                           sorting);
      } else {
...
auto ConjunctionsHeuristic::compute_hcmax_greedy(const State& state) -> int {
  ...
  for (auto conjunction : scheduled_conjunctions) {
    ...
    if (conjunction->is_subgoal && --num_open_goals == 0) return i;
  }
  ...
  while (true) {
    ...
    if (scheduled_conjunctions.empty()) return DEAD_END;
...
auto ConjunctionsHeuristic::compute_hcmax(const State& state) -> int {
  ...
  while (true) {
    ...
    // stop if no new facts are added
    if (next_scheduled_conjunctions.empty()) return DEAD_END;

    std::sort(next_scheduled_conjunctions.begin(),
              next_scheduled_conjunctions.end());
    auto end_iterator = std::unique(next_scheduled_conjunctions.begin(),
                                    next_scheduled_conjunctions.end());

    scheduled_conjunctions.insert(scheduled_conjunctions.begin(),
                                  next_scheduled_conjunctions.begin(),
                                  end_iterator);
    scheduled_conjunctions.resize(
        std::distance(next_scheduled_conjunctions.begin(), end_iterator));
...
void ConjunctionsHeuristic::extract_relaxed_plan() {
  ...
  if (tie_breaking == TieBreaking::CONFLICTS) current_preconditions.clear();
  ...
  // add end node
  const auto end_node_pos =
      current_bsg.add_node(end_action, std::vector<Conjunction*>(),
                           non_dominated_goal_conjunctions, goal_facts);
  ...
  while (!open_conjunctions.empty()) {
    ...
    std::tie(selected_conjunction, selected_action) =
        select_conjunction_and_action(open_conjunctions);
    const auto& selected_counter_group =
        counter_groups[selected_conjunction->counter_groups[selected_action]];
    auto precondition_facts = selected_counter_group.regression;
    auto precondition_conjunctions =
        selected_counter_group.regression_conjunctions;
    auto supported_conjunctions =
        std::vector<Conjunction*>{selected_conjunction};
    ...
    if (!use_singleton_subgoals) {
      ...
      for (auto& open_conjunctions_by_cost : open_conjunctions) {
        for (auto conjunction : open_conjunctions_by_cost.second) {
          // we already added the selected conjunction to the set of supported
          // conjunctions
          if (conjunction == selected_conjunction) continue;

          // selected action is not a best supporter
          if (std::find(conjunction->supporters.begin(),
                        conjunction->supporters.end(),
                        selected_action) == conjunction->supporters.end())
            continue;

          assert(conjunction->counter_groups.find(selected_action) !=
                 std::end(conjunction->counter_groups));
          assert(conjunction->counter_groups[selected_action] !=
                 COUNTER_GROUP_NONE);
          const auto& counter_group =
              counter_groups[conjunction->counter_groups[selected_action]];
          assert(counter_group.value == 0);

          // add new precondition facts
          auto new_precondition_facts = FactSet();
          new_precondition_facts.reserve(precondition_facts.size() +
                                         counter_group.regression.size());
          assert(std::is_sorted(std::begin(precondition_facts),
                                std::end(precondition_facts)));
          assert(std::is_sorted(std::begin(counter_group.regression),
                                std::end(counter_group.regression)));
          std::set_union(std::begin(precondition_facts),
                         std::end(precondition_facts),
                         std::begin(counter_group.regression),
                         std::end(counter_group.regression),
                         std::back_inserter(new_precondition_facts));

          // if preconditions are mutex we cannot add the conjunction
          if (contains_mutex(*task, new_precondition_facts)) continue;

          // if cross context preconditions are enabled, we need to check if we
          // introduced unreachable ones
          if (cross_context) {
            // additional facts needed as preconditions if we add this
            // conjunction
            auto only_new_facts = FactSet();
            std::set_difference(std::begin(new_precondition_facts),
                                std::end(new_precondition_facts),
                                std::begin(precondition_facts),
                                std::end(precondition_facts),
                                std::back_inserter(only_new_facts));

#ifndef NDEBUG
            auto is_new_fact = [&only_new_facts](const auto& f) {
              return std::binary_search(std::begin(only_new_facts),
                                        std::end(only_new_facts), f);
            };
            for (auto precondition_conjunction : precondition_conjunctions)
              assert(std::none_of(std::begin(precondition_conjunction->facts),
                                  std::end(precondition_conjunction->facts),
                                  is_new_fact));
#endif

            // build new set of precondition conjunctions
            auto new_precondition_conjunctions = precondition_conjunctions;
            assert(std::is_sorted(std::begin(new_precondition_conjunctions),
                                  std::end(new_precondition_conjunctions)));

            // add all potential new precondition conjunctions
            for (const auto& fact : only_new_facts) {
              for (auto precondition_conjunction :
                   conjunctions_containing_fact[fact.var][fact.value]) {
                if (!std::binary_search(
                        std::begin(new_precondition_conjunctions) +
                            precondition_conjunctions.size(),
                        std::end(new_precondition_conjunctions),
                        precondition_conjunction) &&
                    std::includes(std::begin(new_precondition_facts),
                                  std::end(new_precondition_facts),
                                  std::begin(precondition_conjunction->facts),
                                  std::end(precondition_conjunction->facts))) {
                  new_precondition_conjunctions.push_back(
                      precondition_conjunction);
                  std::inplace_merge(
                      std::begin(new_precondition_conjunctions) +
                          precondition_conjunctions.size(),
            ...
            std::inplace_merge(std::begin(new_precondition_conjunctions),
                               std::begin(new_precondition_conjunctions) +
                                   precondition_conjunctions.size(),
                               std::end(new_precondition_conjunctions));
            assert(std::is_sorted(std::begin(new_precondition_conjunctions),
                                  std::end(new_precondition_conjunctions)));
            assert(std::unique(std::begin(new_precondition_conjunctions),
                               std::end(new_precondition_conjunctions)) ==
                   std::end(new_precondition_conjunctions));

            // filter dominated conjunctions
            new_precondition_conjunctions =
                get_non_dominated_conjunctions(new_precondition_conjunctions);
            assert(std::is_sorted(std::begin(new_precondition_conjunctions),
                                  std::end(new_precondition_conjunctions)));
            assert(std::unique(std::begin(new_precondition_conjunctions),
                               std::end(new_precondition_conjunctions)) ==
            ...
            auto is_unreachable =
                [selected_conjunction](const Conjunction* conjunction) {
                  return conjunction->cost < 0 ||
                         conjunction->cost >= selected_conjunction->cost;
                };
            if (std::any_of(std::begin(new_precondition_conjunctions),
                            std::end(new_precondition_conjunctions),
                            is_unreachable))
              continue;
            ...
            const auto& regression_conjunctions =
                counter_group.regression_conjunctions;
            precondition_conjunctions.insert(
                std::end(precondition_conjunctions),
                std::begin(regression_conjunctions),
                std::end(regression_conjunctions));
          }
      ...
      // with cross context preconditions disabled we need to filter duplicate
      // and dominated preconditions
      if (!cross_context) {
        // filter duplicates
        std::sort(std::begin(precondition_conjunctions),
                  std::end(precondition_conjunctions));
        precondition_conjunctions.erase(
            std::unique(std::begin(precondition_conjunctions),
                        std::end(precondition_conjunctions)),
            std::end(precondition_conjunctions));

        // filter dominated conjunctions
        precondition_conjunctions =
            get_non_dominated_conjunctions(precondition_conjunctions);
      }
    }
    ...
    std::sort(std::begin(precondition_conjunctions_copy),
              std::end(precondition_conjunctions_copy));
    assert(std::unique(std::begin(precondition_conjunctions_copy),
                       std::end(precondition_conjunctions_copy)) ==
           std::end(precondition_conjunctions_copy));

    // precondition conjunctions sanity check
    if (cross_context) {
      auto expected_precondition_conjunctions = get_non_dominated_conjunctions(
          precondition_facts, conjunctions_containing_fact);
      std::sort(std::begin(expected_precondition_conjunctions),
                std::end(expected_precondition_conjunctions));
      assert(precondition_conjunctions_copy ==
                 std::vector<Conjunction*>{&dummy_precondition} ||
             precondition_conjunctions_copy ==
                 expected_precondition_conjunctions);
    }
#endif

    // build BSG node
    const auto bsg_node_pos =
        current_bsg.add_node(selected_action, supported_conjunctions,
                             precondition_conjunctions, precondition_facts);
    ...
    for (auto* conjunction : supported_conjunctions) {
      ...
      if (tie_breaking == TieBreaking::CONFLICTS) {
        assert(current_preconditions.find(conjunction) !=
               std::end(current_preconditions));
    ...
    for (auto* precondition : precondition_conjunctions) {
      if (!precondition->initially_true) {
        applicable = false;
        auto& open_conjunctions_with_same_cost =
            open_conjunctions[precondition->cost];
        if (std::find(std::begin(open_conjunctions_with_same_cost),
                      std::end(open_conjunctions_with_same_cost),
                      precondition) ==
            std::end(open_conjunctions_with_same_cost))
    ...
    assert(!supported_conjunctions.empty() &&
           "at least the initially selected conjunction should be supported by "
           "the action representative");
    assert(std::find(supported_conjunctions.begin(),
                     supported_conjunctions.end(),
                     selected_conjunction) != supported_conjunctions.end());

    // remove selected conjunctions
    for (auto conjunction : supported_conjunctions) {
      auto open_conjunctions_with_same_cost_it =
          open_conjunctions.find(conjunction->cost);
      assert(
          open_conjunctions_with_same_cost_it != std::end(open_conjunctions) &&
          "the supported conjunctions must be in the set of open conjunctions");
      auto& open_conjunctions_with_same_cost =
          open_conjunctions_with_same_cost_it->second;
      open_conjunctions_with_same_cost.erase(
          std::find(std::begin(open_conjunctions_with_same_cost),
                    std::end(open_conjunctions_with_same_cost), conjunction));
    ...
    if (applicable) {
      current_preferred_operators.push_back(
          selected_action->op->get_global_operator());
#ifndef NDEBUG
      for (const auto& fact : selected_action->pre) {
        assert(conjunctions_containing_fact[fact.var][fact.value]
                   .front()
                   ->facts.size() == 1);
        assert(conjunctions_containing_fact[fact.var][fact.value]
                   .front()
...
auto ConjunctionsHeuristic::select_conjunction_and_action(
    std::map<cost_t, std::vector<Conjunction*>>& open_conjunctions)
  ...
  switch (tie_breaking) {
    ...
    case TieBreaking::DIFFICULTY: {
      ...
      for (auto conjunction : candidate_conjunctions) {
        for (auto supporter : conjunction->supporters) {
          int difficulty = 0;
          for (const auto precondition :
               counter_groups[conjunction->counter_groups[supporter]]
                   .regression_conjunctions)
    ...
    case TieBreaking::SUPPORTED_CONJUNCTIONS: {
      ...
      for (auto* conjunction : candidate_conjunctions) {
        for (auto* supporter : conjunction->supporters) {
          if (supporter == selected_action) continue;
          auto num_supported_conjunctions = 0;
          for (auto& open_conjunctions_by_cost : open_conjunctions)
            for (auto open_conjunction : open_conjunctions_by_cost.second)
              if (std::find(std::begin(open_conjunction->supporters),
                            std::end(open_conjunction->supporters),
                            supporter) !=
                  std::end(open_conjunction->supporters))
      ...
      std::tie(selected_conjunction, selected_action) =
          selections[rng(selections.size())];
      break;
    }
    case TieBreaking::SUPPORTED_CONJUNCTIONS_COST: {
      ...
      for (auto conjunction : candidate_conjunctions) {
        for (auto supporter : conjunction->supporters) {
          ...
          if (supported_conjunctions_cost >
              greatest_supported_conjunctions_cost) {
            selections.clear();
          ...
          if (supported_conjunctions_cost ==
              greatest_supported_conjunctions_cost)
            selections.emplace_back(conjunction, supporter);
          assert(supported_conjunctions_cost <=
                 greatest_supported_conjunctions_cost);
        }
      }
      std::tie(selected_conjunction, selected_action) =
    ...
    case TieBreaking::CONFLICTS: {
      ...
      for (auto* conjunction : candidate_conjunctions) {
        for (auto* supporter : conjunction->supporters) {
          const auto& counter_group =
              counter_groups[conjunction->counter_groups[supporter]];
          const auto num_conflicts = std::count_if(
              std::begin(current_preconditions),
              std::end(current_preconditions),
              [&counter_group, supporter, this](const auto* precondition) {
                return action_representative_edeletes_conjunction(
                    *task, counter_group.regression, supporter->eff,
                    precondition);
              });
  ...
  assert(selected_action &&
         "there must be at least one action that can achieve the conjunction");
  assert(std::find(std::begin(candidate_conjunctions),
                   std::end(candidate_conjunctions),
                   selected_conjunction) != std::end(candidate_conjunctions) &&
         "the selected conjunction must have been in the set of open "
         "conjunctions with the highest cost");
...
void ConjunctionsHeuristic::verify_relaxed_plan(
    const BestSupporterGraph& bsg) const {
  ...
  std::copy_if(std::begin(conjunctions), std::end(conjunctions),
               std::back_inserter(reached), initially_true);
  std::sort(std::begin(reached), std::end(reached));

  // for each action, verify that it is applicable and add the effects to the
  // set of reached conjunctions
  for (const auto& bsg_node : boost::adaptors::reverse(bsg.nodes)) {
    ...
    if (cross_context) {
      ...
      for (const auto supported_conjunction : bsg_node.supported_conjunctions) {
        const auto& regression_facts =
            counter_groups[supported_conjunction
                               ->counter_groups[bsg_node.action]]
                .regression;
        precondition_facts.insert(std::end(precondition_facts),
                                  std::begin(regression_facts),
                                  std::end(regression_facts));
      }
      ...
      precondition_facts.erase(std::unique(std::begin(precondition_facts),
                                           std::end(precondition_facts)),
                               std::end(precondition_facts));
      precondition_conjunctions = get_non_dominated_conjunctions(
          precondition_facts, conjunctions_containing_fact);
    } else {
      for (const auto supported_conjunction : bsg_node.supported_conjunctions) {
        const auto& regression_conjunctions =
            counter_groups[supported_conjunction
                               ->counter_groups[bsg_node.action]]
                .regression_conjunctions;
        precondition_conjunctions.insert(std::end(precondition_conjunctions),
                                         std::begin(regression_conjunctions),
                                         std::end(regression_conjunctions));
      }
      // sort and remove duplicates, then filter dominated conjunctions
    ...
    std::sort(std::begin(precondition_conjunctions),
              std::end(precondition_conjunctions));
    assert(std::includes(std::begin(reached), std::end(reached),
                         std::begin(precondition_conjunctions),
                         std::end(precondition_conjunctions)) &&
    ...
    std::sort(std::begin(reached), std::end(reached));
    assert(std::adjacent_find(reached.begin(), reached.end()) ==
               reached.end() &&
           "reached should be unique");
  }

#ifndef NDEBUG  // avoid unused variable compiler warning
  // verify that all goal conjunctions are reached
  for (const auto goal_conjunction : get_non_dominated_conjunctions(
           end_action->pre, conjunctions_containing_fact))
    assert(
        std::binary_search(reached.begin(), reached.end(), goal_conjunction) &&
        "relaxed plan verification failed: goal conjunction not reached");
#endif
}

auto ConjunctionsHeuristic::get_last_relaxed_plan() const
  ...
  for (const auto& node : boost::adaptors::reverse(current_bsg.nodes)) {
    if (node.action == end_action) break;
...
auto ConjunctionsHeuristic::compute_regressions(const FactSet& facts) const
  ...
  auto regressions =
      std::vector<std::pair<const Action*, std::vector<FactPair>>>();
  const auto potentially_supporting_actions =
      get_potentially_supporting_actions(facts);
  for (const auto* action : potentially_supporting_actions) {
    // mutex pruning included
    if (!is_regressable_and_mutex_free(*action, facts, *task)) continue;
    auto regression = compute_regression(*action, facts);
    // prune regression sets if an easier (i.e. subset) regression already
    // exists
    auto exists_simpler_regression = false;
    // NOTE: A substantial speed up segment. Need an algorithm
    // original run time approximation: O(#Action x #regression x #factSet)
    ...
    // is actually quite expensive, copy it out, and copy it back maybe a better
    // option
    ...
    for (auto other_regression_it = std::begin(regressions);
      ...
      if (regression.size() < other_regression.size()) {
        if (std::includes(std::begin(other_regression),
                          std::end(other_regression), std::begin(regression),
                          std::end(regression)) &&
        ...
        if (std::includes(std::begin(regression), std::end(regression),
                          std::begin(other_regression),
                          std::end(other_regression)) &&
            other_regression_it->first->cost <= action->cost) {
...
void ConjunctionsHeuristic::add_conjunctions(
    const std::vector<FactSet>& factsets) {
  if (factsets.empty()) return;
#ifndef NDEBUG
  for (const auto& facts : factsets) {
    ...
    assert(std::unordered_set<FactSet>(std::begin(factsets), std::end(factsets))
               .size() == factsets.size());
  }
  ...
  for (const auto conjunction : conjunctions) {
    ...
    assert(std::unique(std::begin(regression_of), std::end(regression_of)) ==
           std::end(regression_of));
  }
#endif
  ...
  for (const auto& facts : factsets)
    new_conjunctions.push_back(new Conjunction(
        facts, std::includes(std::begin(goal_facts), std::end(goal_facts),
  ...
  for (auto conjunction : new_conjunctions) {
    ...
    if (!contains_mutex(*task, facts)) {
      ...
      for (auto&& regression : regressions) {
        auto& counter_group_index =
            regression.second.empty()
                ? empty_counter_group_index
                : counter_groups_by_regression[regression.second];
        if (counter_group_index == COUNTER_GROUP_NONE) {
          ...
          if (counter_group.regression.empty()) {
            ...
            counter_group.regression_conjunctions =
                get_non_dominated_conjunctions(counter_group.regression,
                                               conjunctions_containing_fact);
            std::sort(std::begin(counter_group.regression_conjunctions),
                      std::end(counter_group.regression_conjunctions));
            for (auto regression_conjunction :
                 counter_group.regression_conjunctions)
              regression_conjunction->regression_of.push_back(
                  counter_group_index);
          }
        }
        auto& counter_group = counter_groups[counter_group_index];
    ...
    assert(conjunction->is_subgoal ==
           std::includes(std::begin(goal_facts), std::end(goal_facts),
                         std::begin(facts), std::end(facts)));
  }

  if (new_goal_conjunction)
    non_dominated_goal_conjunctions =
        get_non_dominated_conjunctions(goal_conjunctions);

  if (new_conjunctions.size() > 1) {
    // recompute regression conjunctions for counters where any of the new
    // conjunctions would be contained
    auto update_regression_conjunctions = [this, &added_counter_groups,
                                           &new_conjunctions](
        CounterGroupIndex counter_group_index) {
      if (added_counter_groups.find(counter_group_index) !=
          std::end(added_counter_groups))
        return;
      auto& counter_group = counter_groups[counter_group_index];
      assert(std::is_sorted(std::begin(counter_group.regression_conjunctions),
                            std::end(counter_group.regression_conjunctions)));
      if (std::any_of(
              std::begin(new_conjunctions), std::end(new_conjunctions),
              [&counter_group](auto conjunction) {
                return std::includes(std::begin(counter_group.regression),
                                     std::end(counter_group.regression),
                                     std::begin(conjunction->facts),
                                     std::end(conjunction->facts)) &&
                       !std::any_of(
                           std::begin(counter_group.regression_conjunctions),
                           std::end(counter_group.regression_conjunctions),
                           [conjunction](auto regression_conjunction) {
                             return std::includes(
                                 std::begin(regression_conjunction->facts),
                                 std::end(regression_conjunction->facts),
                                 std::begin(conjunction->facts),
                                 std::end(conjunction->facts));
                           });
              })) {
        auto new_regression_conjunctions = get_non_dominated_conjunctions(
            counter_group.regression, conjunctions_containing_fact);
        std::sort(std::begin(new_regression_conjunctions),
                  std::end(new_regression_conjunctions));
        auto& old_regression_conjunctions =
            counter_group.regression_conjunctions;
        assert(std::is_sorted(std::begin(old_regression_conjunctions),
                              std::end(old_regression_conjunctions)));
        // update regression_of
        for (auto old_it = std::begin(old_regression_conjunctions),
                  new_it = std::begin(new_regression_conjunctions);
             !(old_it == std::end(old_regression_conjunctions) &&
               new_it == std::end(new_regression_conjunctions));) {
          if (new_it == std::end(new_regression_conjunctions) ||
              (old_it != std::end(old_regression_conjunctions) &&
               *old_it < *new_it)) {
            // old conjunction is now dominated
            auto& regression_of = (*old_it)->regression_of;
            assert(std::find(std::begin(regression_of), std::end(regression_of),
                             counter_group_index) != std::end(regression_of));
            regression_of.erase(std::find(std::begin(regression_of),
                                          std::end(regression_of),
                                          counter_group_index));
            ++old_it;
          } else if (old_it == std::end(old_regression_conjunctions) ||
                     *old_it > *new_it) {
        ...
        for (auto old_regression_conjunction : old_regression_conjunctions)
          assert(
              std::find(std::begin(new_regression_conjunctions),
                        std::end(new_regression_conjunctions),
                        old_regression_conjunction) !=
                  std::end(new_regression_conjunctions) ||
              std::find(std::begin(old_regression_conjunction->regression_of),
                        std::end(old_regression_conjunction->regression_of),
                        counter_group_index) ==
                  std::end(old_regression_conjunction->regression_of));
        for (auto new_regression_conjunction : new_regression_conjunctions)
          assert(
              std::find(std::begin(new_regression_conjunction->regression_of),
                        std::end(new_regression_conjunction->regression_of),
                        counter_group_index) !=
              std::end(new_regression_conjunction->regression_of));
#endif
        counter_group.regression_conjunctions =
            std::move(new_regression_conjunctions);
        assert(std::is_sorted(std::begin(counter_group.regression_conjunctions),
                              std::end(counter_group.regression_conjunctions)));
      }
    };
    ...
    auto update_regression_conjunctions = [this, conjunction](
        CounterGroupIndex counter_group_index) {
      auto& counter_group = counter_groups[counter_group_index];
      assert(std::is_sorted(std::begin(counter_group.regression_conjunctions),
                            std::end(counter_group.regression_conjunctions)));
      if (counter_group.group.back().second == conjunction) return;
      if (std::includes(std::begin(counter_group.regression),
                        std::end(counter_group.regression),
                        std::begin(conjunction->facts),
                        std::end(conjunction->facts))) {
        /*
                remove dominated conjunctions and only add the new one if no
           other conjunction dominates it
                we can assume that in the set there are no conjunctions that are
           dominated by others, hence there are only two cases:
                1. This conjunction is dominated and will not be added. In this
           case it will also not dominate any other conjunctions.
                2. This conjunction is not dominated and will be added.
        */
        auto is_dominated = false;
        for (auto regression_it =
                 std::begin(counter_group.regression_conjunctions);
             regression_it !=
             std::end(counter_group.regression_conjunctions);) {
          const auto& regression = (*regression_it)->facts;
          if (std::includes(std::begin(conjunction->facts),
                            std::end(conjunction->facts),
            ...
            regression_of.erase(std::find(std::begin(regression_of),
                                          ...
                                          counter_group_index));
            regression_it =
                counter_group.regression_conjunctions.erase(regression_it);
          } else if (std::includes(std::begin(regression), std::end(regression),
                                   std::begin(conjunction->facts),
                                   std::end(conjunction->facts))) {
        ...
        if (!is_dominated) {
          ...
          std::inplace_merge(
              std::begin(counter_group.regression_conjunctions),
              std::end(counter_group.regression_conjunctions) - 1,
              std::end(counter_group.regression_conjunctions));
  ...
  conjunctions.insert(std::end(conjunctions), std::begin(new_conjunctions),
                      std::end(new_conjunctions));
...
auto ConjunctionsHeuristic::get_potentially_supporting_actions(
    const FactSet& facts) const -> std::vector<const Action*> {
  ...
  for (const auto& fact : facts) {
    ...
    potentially_supporting_actions.reserve(
        potentially_supporting_actions.size() + achieving_actions.size());
    const auto old_size = potentially_supporting_actions.size();
    assert(std::is_sorted(std::begin(achieving_actions),
                          std::end(achieving_actions)));
    assert(std::is_sorted(std::begin(potentially_supporting_actions),
                          std::end(potentially_supporting_actions)));
    // if the following assertion didn't hold, the iterators could be
    // invalidated during the insertion in set_difference
  ...
  assert(std::is_sorted(std::begin(potentially_supporting_actions),
                        std::end(potentially_supporting_actions)));
  assert(std::unique(std::begin(potentially_supporting_actions),
                     std::end(potentially_supporting_actions)) ==
         std::end(potentially_supporting_actions));
  return potentially_supporting_actions;
}

void ConjunctionsHeuristic::subscribe(
  ...
  if (inserted) {
    // if for some reason conjunctions were already added, update the novelty
    // heuristics accordingly
    for (auto conjunction_it = std::begin(conjunctions) + num_singletons;
         conjunction_it != std::end(conjunctions); ++conjunction_it)
      novelty_heuristic.add_conjunction((**conjunction_it).facts);
  }
}

void ConjunctionsHeuristic::unsubscribe(
    novelty::NoveltyHeuristic& novelty_heuristic) {
...
auto ConjunctionsHeuristic::get_cost_in_current_state(
    const FactSet& facts) const -> cost_t {
  if (std::all_of(std::begin(facts), std::end(facts), [this](const auto& fact) {
        return current_state_values[fact.var] == fact.value;
      }))
    return 0;
  ...
  auto potentially_supporting_actions =
      get_potentially_supporting_actions(facts);
  for (const auto* action : potentially_supporting_actions) {
    if (is_regressable_and_mutex_free(*action, facts, *task)) {
      auto regression_conjunctions = get_non_dominated_conjunctions(
          compute_regression(*action, facts), conjunctions_containing_fact);
      auto cost =
          best_supporter_function == BestSupporterFunction::HCADD ||
                  best_supporter_function ==
                      BestSupporterFunction::HCADD_ALTERNATIVE
              ? std::accumulate(
                    std::begin(regression_conjunctions),
                    std::end(regression_conjunctions), static_cast<cost_t>(0),
                    [compare](auto cost, auto regression_conjunction) {
                      return std::max({cost, regression_conjunction->cost,
                                       regression_conjunction->cost + cost},
                                      compare);
                    })
              : std::accumulate(
                    std::begin(regression_conjunctions),
...
auto ConjunctionsHeuristic::get_cost_increase_in_current_state(
    const FactSet& facts) const -> cost_t {
  assert(facts.size() > 1);
...
auto ConjunctionsHeuristic::get_num_added_counters(const FactSet& facts) const
    -> int {
  auto adds_counter = [&facts, this](const Action* action) {
    return is_regressable_and_mutex_free(*action, facts, *task);
  };
  const auto potentially_supporting_actions =
      get_potentially_supporting_actions(facts);
  assert(std::count_if(std::begin(potentially_supporting_actions),
                       std::end(potentially_supporting_actions),
                       adds_counter) ==
         std::count_if(std::begin(actions), std::end(actions), adds_counter));
  return std::count_if(std::begin(potentially_supporting_actions),
                       std::end(potentially_supporting_actions), adds_counter);
}

auto ConjunctionsHeuristic::get_num_added_counters_estimate(
    const FactSet& facts) const -> int {
  return get_potentially_supporting_actions(facts).size();
}

auto ConjunctionsHeuristic::get_num_added_counter_groups(
    const FactSet& facts) const -> int {
  const auto potentially_supporting_actions =
      get_potentially_supporting_actions(facts);
  return std::count_if(std::begin(potentially_supporting_actions),
                       std::end(potentially_supporting_actions),
                       [&facts, this](const auto action) {
                         return is_regressable_and_mutex_free(*action, facts,
                                                              *task) &&
                                !counter_groups_by_regression.contains(
                                    compute_regression(*action, facts));
                       });
}

auto ConjunctionsHeuristic::get_num_added_counter_groups_estimate(
    const FactSet& facts) const -> int {
  std::unordered_set<CounterGroupIndex> counter_group_indices;
  for (const auto& f : facts)
    for (const auto& counter_group_pair :
         conjunctions_containing_fact[f.var][f.value].front()->counter_groups)
      counter_group_indices.insert(counter_group_pair.second);
  return counter_group_indices.size();
}

auto ConjunctionsHeuristic::get_added_counter_groups_size(
    const FactSet& facts) const -> int {
  ...
  for (const auto* action : potentially_supporting_actions) {
    ...
    if (!counter_groups_by_regression.contains(regression))
      optimized_added_counter_group_size +=
          get_non_dominated_conjunctions(regression,
                                         conjunctions_containing_fact)
              .size();
  }
  for (const auto& counter_group : counter_groups)
    if (std::includes(std::begin(counter_group.regression),
                      std::end(counter_group.regression), std::begin(facts),
                      std::end(facts)))
      optimized_added_counter_group_size -=
          std::count_if(std::begin(counter_group.regression_conjunctions),
                        std::end(counter_group.regression_conjunctions),
                        [&facts](const auto regression_conjunction) {
                          return std::includes(
                              std::begin(facts), std::end(facts),
                              std::begin(regression_conjunction->facts),
                              std::end(regression_conjunction->facts));
...
auto ConjunctionsHeuristic::get_added_counter_size(const FactSet& facts) const
    -> int {
  auto sum = 0;
  const auto potentially_supporting_actions =
      get_potentially_supporting_actions(facts);
...
auto ConjunctionsHeuristic::get_added_counter_size_after_optimization(
    const FactSet& facts) const -> int {
  ...
  for (const auto& counter_group : counter_groups) {
    if (std::includes(std::begin(counter_group.regression),
                      std::end(counter_group.regression), std::begin(facts),
                      std::end(facts))) {
      removed_counter_size += std::count_if(
          std::begin(counter_group.regression_conjunctions),
          std::end(counter_group.regression_conjunctions),
          [&facts](const auto regression_conjunction) {
            return std::includes(std::begin(regression_conjunction->facts),
                                 std::end(regression_conjunction->facts),
                                 std::begin(facts), std::end(facts));
          });
    }
...
auto ConjunctionsHeuristic::get_num_influenced_counters(
    const FactSet& facts) const -> int {
  return std::accumulate(std::begin(counter_groups), std::end(counter_groups),
                         0, [&facts](auto sum, const auto& counter_group) {
                           return std::includes(
                                      std::begin(counter_group.regression),
                                      std::end(counter_group.regression),
                                      std::begin(facts), std::end(facts))
                                      ? sum
                                      : sum + counter_group.group.size();
                         });
}

auto ConjunctionsHeuristic::get_efficiency(const FactSet& facts) const
    -> double {
  /*
    The number of counters that are influenced by adding this conjunction
    divided by the
    number of new counters introduced when adding this conjunction. This ignores
    the
    possibility that this conjunction might already be dominated by other
    preconditions because
    this is rather complicated to check and happens rarely for new conjunctions
    in practice
  */
  return get_num_influenced_counters(facts) /
         static_cast<double>(get_num_added_counters(facts) + 1);
}

auto ConjunctionsHeuristic::get_efficiency_estimate(const FactSet& facts) const
    -> double {
  /*
    The number of counters that are influenced by adding this conjunction
    divided by the
    estimated number of new counters introduced when adding this conjunction.
    This ignores the
    possibility that this conjunction might already be dominated by other
    preconditions because
    this is rather complicated to check and happens rarely for new conjunctions
  ...
  */
  return get_num_influenced_counters(facts) /
         static_cast<double>(get_num_added_counters_estimate(facts) + 1);
}

auto ConjunctionsHeuristic::get_age(const Conjunction& conjunction) const
    -> int {
  return conjunction.facts.size() > 1
             ? static_cast<int>(std::distance(
                   std::begin(conjunctions) + num_singletons,
                   std::find(std::begin(conjunctions) + num_singletons,
                             std::end(conjunctions), &conjunction)))
             : 0;
}

auto ConjunctionsHeuristic::get_rp_frequency(
    const Conjunction& conjunction) const -> double {
  const auto& statistic = conjunction_statistics.at(&conjunction);
  return statistic.total == 0 ? 1. : statistic.occurences /
                                         static_cast<double>(statistic.total);
}
...
auto ConjunctionsHeuristic::compute_total_counter_group_size() const -> int {
  return std::accumulate(std::begin(counter_groups), std::end(counter_groups),
                         0, [](auto sum, const auto& counter_group) {
                           return sum +
                                  counter_group.regression_conjunctions.size();
                         });
}
...
void ConjunctionsHeuristic::remove_all_conjunctions() {
  ...
  for (auto i = 0; i < static_cast<int>(g_variable_domain.size()); ++i)
    for (auto j = 0; j < g_variable_domain[i]; ++j)
      conjunctions_containing_fact[i][j].erase(
          std::begin(conjunctions_containing_fact[i][j]) + 1,
          std::end(conjunctions_containing_fact[i][j]));

  // clear regression_of
  for (auto singleton_conjunction_it = std::begin(conjunctions);
       singleton_conjunction_it != std::begin(conjunctions) + num_singletons;
       ++singleton_conjunction_it)
    (*singleton_conjunction_it)->regression_of.clear();

  // rebuild regression_conjunctions and regression_of
  for (auto counter_group_index = static_cast<CounterGroupIndex>(0);
       counter_group_index <
       static_cast<CounterGroupIndex>(num_singleton_counter_groups);
       ++counter_group_index) {
    auto& counter_group = counter_groups[counter_group_index];
    assert(std::is_sorted(
        std::begin(counter_group.group), std::end(counter_group.group),
        [](const auto& lhs, const auto& rhs) {
          return lhs.second->facts.size() <= 1 && rhs.second->facts.size() > 1;
        }));
    counter_group.group.erase(
        std::upper_bound(
            std::begin(counter_group.group), std::end(counter_group.group), 1u,
            [](auto conjunction_size, const auto& group_member) {
              return conjunction_size < group_member.second->facts.size();
            }),
        std::end(counter_group.group));
    assert(std::none_of(std::begin(counter_group.group),
                        std::end(counter_group.group),
                        [](const auto& group_member) {
                          return group_member.second->facts.size() > 1;
                        }));
    // if the regression is empty only the dummy precondition is in the
    // regression conjunctions, which is kept as is
    if (!counter_group.regression.empty()) {
      ...
      for (const auto& f : counter_group.regression) {
        auto regression_singleton =
            conjunctions_containing_fact[f.var][f.value].front();
...
#ifndef NDEBUG
    auto expected_regression_conjunctions = get_non_dominated_conjunctions(
        counter_group.regression, conjunctions_containing_fact);
    auto actual_regression_conjunctions = counter_group.regression_conjunctions;
    std::sort(std::begin(expected_regression_conjunctions),
              std::end(expected_regression_conjunctions));
    std::sort(std::begin(actual_regression_conjunctions),
              std::end(actual_regression_conjunctions));
    assert(expected_regression_conjunctions == actual_regression_conjunctions ||
           actual_regression_conjunctions ==
               std::vector<Conjunction*>{&dummy_precondition});
#endif
  }
  ...
  for (auto counter_group_index =
           static_cast<CounterGroupIndex>(num_singleton_counter_groups);
       counter_group_index < counter_groups.size(); ++counter_group_index) {
    if (unused_counter_groups_iterator != std::end(unused_counter_groups) &&
        *unused_counter_groups_iterator == counter_group_index) {
      ++unused_counter_groups_iterator;
      continue;
    }
    counter_groups_by_regression.remove(
        counter_groups[counter_group_index].regression);
  }
  counter_groups.erase(
      std::begin(counter_groups) + num_singleton_counter_groups,
      std::end(counter_groups));
  ...
  while (first_non_singleton_goal != std::end(goal_conjunctions) &&
         (*first_non_singleton_goal)->facts.size() == 1)
    ++first_non_singleton_goal;

  // reinitialize goal conjunctions
  goal_conjunctions.erase(first_non_singleton_goal,
                          std::end(goal_conjunctions));
  ...
  conjunctions.erase(std::begin(conjunctions) + num_singletons,
                     std::end(conjunctions));
...
void ConjunctionsHeuristic::remove_conjunction(Conjunction* conjunction) {
  ...
  conjunctions.erase(
      std::find(std::begin(conjunctions), std::end(conjunctions), conjunction));
  conjunction_statistics.erase(conjunction);
  for (const auto& f : conjunction->facts) {
    auto& conjunctions_containing_f =
        conjunctions_containing_fact[f.var][f.value];
    conjunctions_containing_f.erase(
        std::find(std::begin(conjunctions_containing_f),
                  std::end(conjunctions_containing_f), conjunction));
  }
  ...
  for (auto& counter_group_pair : conjunction->counter_groups) {
    ...
    if (counter_group.group.size() == 1) {
      ...
      std::inplace_merge(std::begin(removed_counter_groups),
                         std::end(removed_counter_groups) - 1,
                         std::end(removed_counter_groups));
    } else {
      assert(std::find(std::begin(counter_group.group),
                       std::end(counter_group.group),
                       std::make_pair(counter_group_pair.first, conjunction)) !=
             std::end(counter_group.group));
      counter_group.group.erase(std::find(
          std::begin(counter_group.group), std::end(counter_group.group),
          std::make_pair(counter_group_pair.first, conjunction)));
    }
  }
  ...
  for (auto removed_counter_group_index : removed_counter_groups) {
    for (auto regression_conjunction :
         counter_groups[removed_counter_group_index].regression_conjunctions) {
      regression_conjunction->regression_of.erase(
          std::find(std::begin(regression_conjunction->regression_of),
                    std::end(regression_conjunction->regression_of),
                    removed_counter_group_index));
    }
    counter_groups_by_regression.remove(
        counter_groups[removed_counter_group_index].regression);
    remove_counter_group(removed_counter_group_index);
  }

#ifndef NDEBUG
  auto check_counter_group_has_conjunction =
      [conjunction](const auto& counter_group) {
        assert(std::none_of(std::begin(counter_group.group),
                            std::end(counter_group.group),
                            [conjunction](const auto& counter_group_pair) {
                              return counter_group_pair.second == conjunction;
                            }));
      };
  ...
  for (auto counter_group_index : conjunction->regression_of) {
    auto& regression_conjunctions =
        counter_groups[counter_group_index].regression_conjunctions;
    assert(std::find(std::begin(regression_conjunctions),
                     std::end(regression_conjunctions),
                     conjunction) != std::end(regression_conjunctions));
    regression_conjunctions.erase(std::find(std::begin(regression_conjunctions),
                                            std::end(regression_conjunctions),
                                            conjunction));
    auto potential_regression_conjunctions = get_non_dominated_conjunctions(
        conjunction->facts, conjunctions_containing_fact);
    for (auto potential_regression_conjunction :
         potential_regression_conjunctions) {
      auto dominates_conjunction =
          [potential_regression_conjunction](const Conjunction* other) {
            return potential_regression_conjunction != other &&
                   std::includes(
                       std::begin(other->facts), std::end(other->facts),
                       std::begin(potential_regression_conjunction->facts),
                       std::end(potential_regression_conjunction->facts));
          };
      if (std::none_of(std::begin(regression_conjunctions),
                       std::end(regression_conjunctions),
                       dominates_conjunction)) {
        regression_conjunctions.push_back(potential_regression_conjunction);
        std::inplace_merge(std::begin(regression_conjunctions),
                           std::end(regression_conjunctions) - 1,
                           std::end(regression_conjunctions));
        assert(std::find(
                   std::begin(potential_regression_conjunction->regression_of),
                   std::end(potential_regression_conjunction->regression_of),
                   counter_group_index) ==
               std::end(potential_regression_conjunction->regression_of));
        potential_regression_conjunction->regression_of.push_back(
            counter_group_index);
      }
    }
#ifndef NDEBUG
    ...
    std::sort(std::begin(expected_regression_conjunctions),
              std::end(expected_regression_conjunctions));
    assert(std::is_sorted(std::begin(regression_conjunctions),
                          std::end(regression_conjunctions)));
    assert(expected_regression_conjunctions == regression_conjunctions ||
           regression_conjunctions ==
               std::vector<Conjunction*>{&dummy_precondition});
#endif
  }
  ...
  for (const auto other_conjunction : conjunctions) {
    for (const auto& counter_group_pair : other_conjunction->counter_groups) {
      const auto& regression_conjunctions =
          counter_groups[counter_group_pair.second].regression_conjunctions;
      assert(std::find(std::begin(regression_conjunctions),
                       std::end(regression_conjunctions),
                       conjunction) == std::end(regression_conjunctions));
    }
    for (const auto counter_group_index : removed_counter_groups)
      assert(std::find(std::begin(other_conjunction->regression_of),
                       std::end(other_conjunction->regression_of),
                       counter_group_index) ==
             std::end(other_conjunction->regression_of));
    ...
    for (auto removed_counter_group_index : removed_counter_groups)
      assert(std::find(std::begin(regression_of), std::end(regression_of),
                       removed_counter_group_index) == std::end(regression_of));
  }
  auto check_counter_group = [this](CounterGroupIndex counter_group_index) {
    auto& counter_group = counter_groups[counter_group_index];
    for (const auto& counter_group_pair : counter_group.group) {
      auto expected_regression_conjunctions = get_non_dominated_conjunctions(
          compute_regression(*counter_group_pair.first,
                             counter_group_pair.second->facts),
          conjunctions_containing_fact);
      std::sort(std::begin(expected_regression_conjunctions),
                std::end(expected_regression_conjunctions));
      assert(counter_group.regression_conjunctions ==
             expected_regression_conjunctions);
    }
    for (const auto regression_conjunction :
         counter_group.regression_conjunctions)
      assert(std::find(std::begin(regression_conjunction->regression_of),
                       std::end(regression_conjunction->regression_of),
                       counter_group_index) !=
             std::end(regression_conjunction->regression_of));
  };
  ...
  if (conjunction->is_subgoal) {
    ...
    assert(std::find(std::begin(goal_conjunctions), std::end(goal_conjunctions),
                     conjunction) != std::end(goal_conjunctions));
    goal_conjunctions.erase(std::find(std::begin(goal_conjunctions),
                                      std::end(goal_conjunctions),
                                      conjunction));

    // ... and also from the non_dominated_goal_conjunctions set if it is
    // contained there
    auto deleted_conjunction_pos =
        std::find(std::begin(non_dominated_goal_conjunctions),
                  std::end(non_dominated_goal_conjunctions), conjunction);
    if (deleted_conjunction_pos != std::end(non_dominated_goal_conjunctions)) {
      ...
      auto potential_goal_conjunctions = get_non_dominated_conjunctions(
          conjunction->facts, conjunctions_containing_fact);
      for (auto potential_goal_conjunction : potential_goal_conjunctions) {
        auto dominates_conjunction = [potential_goal_conjunction](
            const Conjunction* other) {
          return potential_goal_conjunction != other &&
                 std::includes(std::begin(other->facts), std::end(other->facts),
                               std::begin(potential_goal_conjunction->facts),
                               std::end(potential_goal_conjunction->facts));
        };
        if (std::none_of(std::begin(non_dominated_goal_conjunctions),
                         std::end(non_dominated_goal_conjunctions),
                         dominates_conjunction))
...
void ConjunctionsHeuristic::remove_random_conjunctions(int count,
                                                       int min_evaluations) {
  if (min_evaluations == 0) {
    auto actual_count =
        std::min<int>(count, conjunctions.size() - num_singletons);
    ...
    auto conjunctions_copy = std::vector<Conjunction*>(
        std::begin(conjunctions) + num_singletons, std::end(conjunctions));
    rng.shuffle(conjunctions_copy);
    auto removed = 0;
    auto skipped = 0;
    for (auto i = 0;
         i < count + skipped && i < static_cast<int>(conjunctions_copy.size());
         ++i) {
    ...
    if (removed < count) remove_random_conjunctions(count - removed, 0);
  }
}
...
void ConjunctionsHeuristic::remove_oldest_conjunctions(int count, int) {
  ...
  for (auto i = 0; i < count; ++i) {
    ...
    assert(conjunction->facts.size() > 1);
    assert(std::is_sorted(std::begin(conjunctions) + num_singletons,
                          std::end(conjunctions),
                          [this](const auto lhs, const auto rhs) {
                            return conjunction_statistics[lhs].total >
                                   conjunction_statistics[rhs].total;
                          }));
    remove_conjunction(conjunction);
  }
}

void ConjunctionsHeuristic::remove_conjunctions_by_score(
    std::function<double(const Conjunction*)> score, const bool remove_lowest,
    int count, int min_evaluations) {
  ...
  if (remove_lowest)
    std::sort(std::begin(conjunctions_tmp), std::end(conjunctions_tmp),
              [](const auto& lhs, const auto& rhs) {
...
void ConjunctionsHeuristic::
    remove_least_frequently_in_relaxed_plans_conjunctions(int count,
                                                          int min_evaluations) {
  ...
  remove_conjunctions_by_score(relaxed_plan_frequency, true, count,
                               min_evaluations);
}

void ConjunctionsHeuristic::remove_least_efficient_conjunctions(
    int count, int min_evaluations) {
  auto efficiency = [this](const Conjunction* conjunction) {
    return conjunction_statistics[conjunction].total == 0
               ? std::numeric_limits<double>::max()
               : (conjunction->regression_of.size() + 1) /
                     static_cast<double>(conjunction->counter_groups.size() +
                                         1);
  };
  remove_conjunctions_by_score(efficiency, true, count, min_evaluations);
}

void ConjunctionsHeuristic::remove_conjunctions_with_most_counters(
    int count, int min_evaluations) {
  auto count_counters = [this,
                         min_evaluations](const Conjunction* conjunction) {
    return conjunction_statistics[conjunction].total < min_evaluations
               ? -1
               : static_cast<int>(conjunction->counter_groups.size());
  };
...
auto operator<<(std::ostream& out,
                const ConjunctionsHeuristic::BestSupporterFunction bs)
    -> std::ostream& {
  switch (bs) {
    ...
    default: {
      std::cout << "Unknown best supporter function option: " << bs
                << std::endl;
      utils::exit_with(utils::ExitCode::CRITICAL_ERROR);
    }
  }
...
auto operator<<(std::ostream& out, const ConjunctionsHeuristic::TieBreaking tb)
    -> std::ostream& {
  switch (tb) {
...
void ConjunctionsHeuristic::add_common_options(options::OptionParser& parser) {
  parser.add_option<std::shared_ptr<ConjunctionGenerationStrategy>>(
      "strategy",
      "Conjunction generation strategy to be used. Local minima are not "
      "tracked. "
      "This option should only be used if the conjunction is used in a "
      "standard search algorithm. Consider using a search algorithm "
      "specialized for conjunctions instead.",
...
static auto _parse(options::OptionParser& parser) -> Heuristic* {
  ...
  parser.document_language_support("conditional effects",
                                   "supported (removed by enumeration)");
  ...
  parser.add_option<bool>("cross_context", "Use cross context preconditions.",
                          "true");
  parser.add_option<bool>("use_singleton_subgoals",
                          "Only use singleton subgoal sets.", "false");
  parser.add_option<bool>(
      "use_action_sets",
      "Count actions instead of action/conjunctions pairs in the relaxed plan.",
      "false");

  // best supporter functions
  const auto best_supporter_options = std::vector<std::string>{
      "HCADD", "HCADD_ALTERNATIVE", "HCMAX", "HCMAX_GREEDY"};
  parser.add_enum_option(
      "best_supporter", best_supporter_options,
      "Choose either h^Cadd or h^Cmax as the best supporter function. "
      "HCADD_ALTERNATIVE is an alternative implementation of h^Cadd that is "
      "generally slower than the other one, but may be quicker in some cases "
      "(e.g. in the visitall domain). "
      "HCMAX_GREEDY is a faster implementation of h^Cmax, but only one action "
      "is selected as the best supporter for each conjunction which makes the "
      "relaxed plan extraction less flexible.",
      "HCADD");
  ...
  /*
    ...
    note the tie breaking/best supporter choices are a bit restricted:
      ...
      for the faster h^Cmax implementation, the tie breaking option does not do
    anything since there is never a tie breaking between multiple actions
  */
  const auto tie_breaking_options =
  ...
  parser.add_enum_option("tie_breaking", tie_breaking_options,
                         "best supporter selection tie breaking", "RANDOM",
                         {"arbitrary",
                          "break ties by the sum of the cost of the "
                          "preconditions (like FF, only usable with h^Cmax)",
                          "random",
                          "break ties by the number of open conjunctions that "
                          "can be achieved by the selected action",
                          "same as previous, but use the sum of the cost of "
                          "the supported conjunctions",
                          "minimize conflicts in the relaxed plan"});
  parser.add_option<int>("seed",
                         "Random seed (only used for random tie breaking). If "
                         "this is set to -1, an arbitrary seed is used.",
                         "-1");
...
static auto _parse_add(options::OptionParser& parser) -> Heuristic* {
  ...
  Heuristic::add_options_to_parser(parser);
  ConjunctionsHeuristic::add_common_options(parser);
  parser.add_option<bool>("use_alternative_implementation",
                          "Use an alternative h^Cadd implementation using "
                          "different data structures. "
                          "This implementation is generally slower than the "
                          "default one, but may be quicker in some cases (e.g. "
                          "in the visitall domain).",
                          "false");

  options::Options opts = parser.parse();
  opts.set("best_supporter",
           opts.get<bool>("use_alternative_implementation") ? 1 : 0);
  ConjunctionsHeuristic::set_hc_options(opts);
