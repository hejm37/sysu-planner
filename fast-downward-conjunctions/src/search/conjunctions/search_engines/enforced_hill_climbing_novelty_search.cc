...
EnforcedHillClimbingNoveltySearch::EnforcedHillClimbingNoveltySearch(
    ...
    : OnlineLearningSearchEngine(opts),
      ...
      novelty_heuristic(static_cast<novelty::NoveltyHeuristic*>(
          opts.get<Heuristic*>("novelty"))),
      ...
      learning_stagnation_threshold(
          opts.get<int>("learning_stagnation_threshold")),
  ...
  if (!use_preferred || preferred_usage == PreferredUsage::PRUNE_BY_PREFERRED) {
    ...
    auto evals = std::vector<ScalarEvaluator*>{
        g_evaluator, new pref_evaluator::PrefEvaluator(), heuristic};
    Options options;
  ...
  if (unsafe_pruning_sse &&
      search_space_exhaustion == SearchSpaceExhaustion::CONTINUE)
    std::cout << "Warning: using CONTINUE on search space exhaustion, unsafe "
                 "pruning has no effect."
              << std::endl;
  if (unsafe_pruning_ls && learning_stagnation_threshold > 100)
    std::cout << "Warning: using a very high threshold for learning "
                 "stagnation, unsafe pruning probably has no effect."
              << std::endl;
  assert(!enable_heuristic_cache ||
         (use_preferred && preferred_operator_heuristics.size() == 1));
}

EnforcedHillClimbingNoveltySearch::~EnforcedHillClimbingNoveltySearch() {}

void EnforcedHillClimbingNoveltySearch::reach_state(const GlobalState& parent,
...
void EnforcedHillClimbingNoveltySearch::initialize() {
  ...
  assert(heuristic);
  std::cout << "Conducting enforced hill-climbing search with explicit "
               "conjunctions, (real) bound = "
            << bound << std::endl;
  if (use_preferred)
  ...
  std::cout << "Always reevaluate the local minimum neighborhood after adding "
               "a conjunction: "
  ...
  solved = generate_conjunctions(
               *heuristic, ConjunctionGenerationStrategy::Event::INITIALIZATION,
               current_eval_context, true,
               bound) == ConjunctionGenerationStrategy::Result::SOLVED &&
           heuristic->get_last_bsg().get_real_cost() <= bound;
  heuristic->print_statistics();
  std::cout << "Finished initialization, t = " << initialization_timer
            << std::endl;
  ...
  assert(
      heuristic->is_last_bsg_valid_for_state(current_eval_context.get_state()));
  if (!solved && check_relaxed_plans &&
      is_valid_plan_in_the_original_task(
          heuristic->get_last_bsg(),
          current_eval_context.get_state().get_values(), *g_root_task()) &&
      heuristic->get_last_bsg().get_real_cost() <= bound) {
    solved = true;
    set_solution(heuristic->get_last_relaxed_plan(),
                 current_eval_context.get_state());
  }
}

auto EnforcedHillClimbingNoveltySearch::get_successors(
    EvaluationContext& eval_context) -> std::vector<const GlobalOperator*> {
  ...
  if (!use_preferred ||
    ...
    if (!enable_heuristic_cache) {
      ...
      assert(heuristic_cache.find(eval_context.get_state().get_id()) !=
...
void EnforcedHillClimbingNoveltySearch::expand(
    EvaluationContext& eval_context, SearchSpace& current_search_space) {
...
SearchStatus EnforcedHillClimbingNoveltySearch::ehc(
  ...
  while (!open_list->empty()) {
    ...
    if (node.is_new()) {
      ...
      auto is_novel = [this, &eval_context]() {
        ...
        const auto novel =
            eval_context.get_heuristic_value(novelty_heuristic) == 0;
      ...
      if (novelty_heuristic->is_basic() && !is_novel()) continue;
      ...
      if (check_relaxed_plans &&
          ...
          is_valid_plan_in_the_original_task(
              heuristic->get_last_bsg(), state.get_values(), *g_root_task()) &&
          heuristic->get_last_bsg().get_real_cost() + current_real_g +
                  parent_node.get_real_g() + last_op->get_cost() <=
              bound) {
        auto path_to_current_state = Plan();
        search_space.trace_path(current_eval_context.get_state(),
                                path_to_current_state);
        ...
        solution.insert(std::end(solution), std::begin(path_to_local_state),
                        std::end(path_to_local_state));
        solution.insert(std::end(solution), std::begin(relaxed_plan),
                        std::end(relaxed_plan));
        set_plan(solution);
        return SOLVED;
      }
  ...
  if (!bfs_cutoff && preferred_usage != PreferredUsage::PRUNE_BY_PREFERRED &&
      !dead_end_unsafe) {
...
void EnforcedHillClimbingNoveltySearch::update_eval_context(
  ...
  // the magic number -1 in the following line corresponds to
  // EvaluationContext::INVALID, which is private :(
  eval_context = EvaluationContext(cache, -1, false, &statistics);
}

auto EnforcedHillClimbingNoveltySearch::evaluate_if_neccessary(
...
auto EnforcedHillClimbingNoveltySearch::escape_local_minimum(
  ...
  ehcc_statistics.max_stagnation_count = std::max(
      ehcc_statistics.max_stagnation_count, learning_stagnation_counter);
  ...
  if (learning_stagnation_counter >= learning_stagnation_threshold) {
    // learning is called in the same phase/state as the last time, i.e. we
    // could not find a better state after improving the heuristic
  ...
  do {
    ...
    current_eval_context =
        EvaluationContext(current_eval_context.get_state(), &statistics);
    auto learning_result = generate_conjunctions(
        *heuristic, ConjunctionGenerationStrategy::Event::LOCAL_MINIMUM,
        current_eval_context, true, bound - current_real_g);
    ...
    assert(
        learning_result == ConjunctionGenerationStrategy::Result::MODIFIED ||
...
auto EnforcedHillClimbingNoveltySearch::handle_safe_dead_end() -> SearchStatus {
  ...
  if (!current_unsafe_dead_ends.empty()) {
    for (const auto state_id : current_unsafe_dead_ends) {
      auto unsafe_dead_end_node =
          search_space.get_node(state_registry.lookup_state(state_id));
...
auto EnforcedHillClimbingNoveltySearch::handle_search_space_exhaustion()
    -> SearchStatus {
  ...
  assert(search_space_exhaustion == SearchSpaceExhaustion::RESTART ||
         search_space_exhaustion == SearchSpaceExhaustion::BACKJUMP);
  if (unsafe_pruning_sse) {
    if (force_unsafe_pruning_complete) {
      ...
      node.mark_as_dead_end();
      statistics.inc_dead_ends();
      return search_space_exhaustion == SearchSpaceExhaustion::RESTART
                 ? restart()
  ...
  return search_space_exhaustion == SearchSpaceExhaustion::RESTART
             ? restart()
             : escape_potential_dead_end();
}

auto EnforcedHillClimbingNoveltySearch::escape_dead_end(const SearchNode& node)
    -> SearchStatus {
  ...
  current_eval_context =
      EvaluationContext(state_registry.lookup_state(parent_id), &statistics);
...
auto EnforcedHillClimbingNoveltySearch::escape_potential_dead_end()
    -> SearchStatus {
  auto node = search_space.get_node(current_eval_context.get_state());
  assert(!node.is_dead_end());
  learning_stagnation_counter = 0;
...
auto EnforcedHillClimbingNoveltySearch::restart() -> SearchStatus {
  learning_stagnation_counter = 0;
  current_eval_context =
      EvaluationContext(state_registry.get_initial_state(), &statistics);
  current_real_g = 0;
  auto h = evaluate_if_neccessary(current_eval_context);
  if (h == EvaluationResult::INFTY) {
    std::cout << "Initial state is a dead end, no solution" << std::endl;
...
auto EnforcedHillClimbingNoveltySearch::restart_in_parent() -> SearchStatus {
  assert(current_eval_context.get_state().get_id() !=
         state_registry.get_initial_state().get_id());
  learning_stagnation_counter = 0;
  auto node = search_space.get_node(current_eval_context.get_state());
...
void EnforcedHillClimbingNoveltySearch::print_ehcc_statistics() const {
  ...
  std::cout << "Average expansions per EHC phase: "
            << ehcc_statistics.get_avg_expansions_per_ehc_phase(
                   statistics.get_expanded())
            << std::endl;
  std::cout << "Dead ends during learning: "
            << ehcc_statistics.num_dead_ends_during_learning << std::endl;
  std::cout << "Dead ends during learning or backjumping: "
            << ehcc_statistics.num_dead_ends << std::endl;
  std::cout << "No better state after learning: "
            << ehcc_statistics.num_no_better_state_after_learning << std::endl;
  std::cout << "Max learning stagnation count: "
            << ehcc_statistics.max_stagnation_count << std::endl;
  std::cout << "Average learning stagnation count: "
            << ehcc_statistics.get_avg_stagnation_count(
                   online_learning_statistics.num_learning_calls)
            << std::endl;
  std::cout << "Evaluations saved by buffering: "
            << ehcc_statistics.num_saved_evaluations << std::endl;
  std::cout << "Exhausted search space: "
            << ehcc_statistics.num_search_space_exhaustion << std::endl;
  std::cout << "Average dead end backjump length: "
            << (ehcc_statistics.num_dead_ends != 0
                    ? ehcc_statistics.total_dead_end_backjump_length /
                          static_cast<double>(ehcc_statistics.num_dead_ends)
                    : 0.0)
            << std::endl;
  std::cout << "Total time spent on evaluating novelty: "
            << ehcc_statistics.novelty_timer() << std::endl;
}

void EnforcedHillClimbingNoveltySearch::print_intermediate_statistics(
...
void EnforcedHillClimbingNoveltySearch::print_statistics() const {
  ...
  for (const auto& count : d_counts) {
    ...
    std::cout << "EHC phases of depth " << depth << ": " << phases
              << " - Avg. Expansions: "
...
static auto _parse(OptionParser& parser) -> SearchEngine* {
  ...
  parser.add_option<Heuristic*>("novelty",
                                "novelty heuristic used for pruning in BFS "
                                "explorations, must be of type "
                                "NoveltyHeuristic",
                                "novelty(cache_estimates=false)");
  parser.add_option<bool>("no_learning", "exit instead of learning", "false");
  parser.add_option<bool>("restart_in_dead_ends",
                          "Do a full restart if the current state is safely "
                          "recognized as a dead end; "
                          "otherwise just go back to the last state along the "
                          "path that is not the current state and does not "
                          "have heuristic value infinity. This can happen in "
                          "two ways: "
                          "1. Without learning, the entire local search space "
                          "is exhausted around the current best state. "
                          "2. The current best state might have heuristic "
                          "value infinity after learning new conjunctions.",
                          "true");
  parser.add_option<int>(
      "bfs_bound", "lookahead bound for breadth first search", "infinity");
  parser.add_option<bool>("always_reevaluate",
                          "always reevaluate the local minimum neighborhood "
                          "after adding a conjunction",
                          "false");
  ...
  parser.add_option<int>("seed",
                         "Random seed (for successor randomization). If this "
                         "is set to -1, an arbitrary seed is used.",
                         "-1");
  parser.add_option<int>("learning_stagnation_threshold",
                         "how often learning must be repeated in the same "
                         "state to trigger a learning stagnation",
  ...
  parser.add_enum_option(
      ...
      "RESTART", {"With learning, continue with the next BFS phase at the "
                  ...
                  "learning, search fails in this case.",
                  ...
                  "Like backjump, but mark this state as an unsafe dead end. "
                  "Unsafe dead ends are handled like dead ends (i.e. never "
  ...
  parser.add_option<bool>("unsafe_pruning_sse",
                          "mark states as dead end on search space exhaustion",
                          "false");
  parser.add_option<bool>("unsafe_pruning_ls",
                          "mark states as dead end on learning stagnation",
                          "false");
  parser.add_option<bool>("force_unsafe_pruning_complete",
                          "enforce completeness when using unsafe pruning",
                          "false");
