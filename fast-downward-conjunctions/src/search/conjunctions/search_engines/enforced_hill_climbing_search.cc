...
EnforcedHillClimbingSearch::EnforcedHillClimbingSearch(
  ...
  if (unsafe_pruning_sse &&
    ...
    std::cout << "Warning: using CONTINUE on search space exhaustion, "
                 "unsafe pruning has no effect."
              << std::endl;
...
auto EnforcedHillClimbingSearch::proceed_with_no_better_state(
  ...
  return IN_PROGRESS;
  // auto current_node =
  // search_space.get_node(current_eval_context.get_state());
  // // excluded_states.insert(current_node.get_state_id());
  // current_eval_context =
  //   EvaluationContext(state_registry.lookup_state(next_best_state_id),
  //   &statistics);
...
/*
 * Option Continue: continue learning by passing IN_PROGRESS, or fail if no
 * learning
 * Option Restart or BackJump:
 * If unsafe_pruning_sse: need to mark unsafe_dead_end or dead_end
 * Then decide restart or backjump
 */
auto EnforcedHillClimbingSearch::handle_search_space_exhaustion()
    -> SearchStatus {
...
// not because of k_cutoff and not explore all possible
// node(current_unsafe_dead_ends not empty)
