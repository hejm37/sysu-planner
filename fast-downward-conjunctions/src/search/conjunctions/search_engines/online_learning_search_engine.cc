...
auto OnlineLearningSearchEngine::generate_conjunctions(
  ConjunctionsHeuristic &heuristic,
  ConjunctionGenerationStrategy::Event event,
  EvaluationContext &eval_context,
  bool check_solved,
  int bound) -> ConjunctionGenerationStrategy::Result {
  ...
  // this check is for modifying conjunctions make the heuristic evaluate to 0
  // therefore a solution found
	if (check_solved &&
      result == ConjunctionGenerationStrategy::Result::SOLVED &&
      (bound == -1 || heuristic.get_last_bsg().get_real_cost() <= bound)) {
